...
class Uploader {
  ...
  static reqToOptions (req, size) {
    ...
    logger.debug("check", JSON.stringify(req.body.metadata))
    logger.debug("check", JSON.stringify(req.companion.options.providerOptions.s3))
    ...
    //logger.debug("formData", useFormData)
    return {
  ...
  /**
  * sets s3 options and clear metadata from file
  */
  setS3Options(req) {
  ...
  uploadTus () {
    ...
    const uploader = this
    logger.debug("check", this.options)
